/**
 * Author: Margherita Donnici e Sara Zuppiroli
 */
...
var slept = false;
var returnedInHotel=false;
var returnedInHotelInHour=false;
...
$(document).ready(function(){
    ...
    for (i=0; i<locationArray.length; i++){
        ...
        if (level > 0 && showButtonInfo(i)){
            ...
            //aggiunto l'eveonto sul puslante
...
function moveToLocation(event, container) {
    loc = getLocationFromId(event.currentTarget.id);
    if (loc.currentlyAccessible == true) {
...
function updateTime(){
    ...
    for (i=0; i < timedGoals.length; i++){
        if (goalList.hasOwnProperty(timedGoals[i].id) && !goalList[timedGoals[i].id].completed && !goalList[timedGoals[i].id].notifiedFail && currentTime > timedGoals[i].time){
        	showSnackbar(timedGoals[i].failMsg);
					...
					case "Incontra amici":
						for (let j=14; j<20; j++){
							...
							if (goalList.hasOwnProperty(j) && currentTime == goal.time){
								...
								newButton.appendChild(text);
								newButton.addEventListener('click', function(){
								    doActivityTimedAction(location, "Chiacchierando...");
								});
								$("#topToolbar").append(newButton);	
							}
						}
...
function doActivityTimedAction(location, activity){
	    ...
	    setTimeout(function(){
	    	dialog.modal('hide');
		    // Check if a goal was satisfied and notify
		   if (activity == "Mangiando..." && currentTime == 12){
			   notifySatisfiedGoal(14);
		   } else if (activity == "Mangiando..." && currentTime == 13){
			   notifySatisfiedGoal(15);
		   } else if (activity == "Mangiando..." && currentTime == 36){
			   notifySatisfiedGoal(16);
		   } else if (activity == "Mangiando..." && currentTime == 37){
			   notifySatisfiedGoal(17);
		   } else if (activity == "Mangiando..." && currentTime == 20){
			   notifySatisfiedGoal(18);
		   } else if (activity == "Mangiando..." && currentTime == 21){
			   notifySatisfiedGoal(19);
		   } else if (activity == "Bevendo una birra..." && currentTime == 22){
			   notifySatisfiedGoal(20);
		   } else if (activity == "Bevendo una birra..." && currentTime == 23){
			   notifySatisfiedGoal(21);
		   } else if (activity == "Bevendo una birra..." && currentTime == 42){
			   notifySatisfiedGoal(22);
		   } else if (activity == "Bevendo una birra..." && currentTime == 43){
			   notifySatisfiedGoal(23);
		   } else if (activity == "Guardando la partita..." && currentTime == 14){
			   notifySatisfiedGoal(24);
		   } else if (activity == "Guardando la partita..." && currentTime == 20){
			   notifySatisfiedGoal(25);
		   } else if (activity == "Guardando la partita..." && currentTime == 38){
			   notifySatisfiedGoal(26);
		   } else if (activity == "Guardando la partita..." && currentTime == 44){
		   ...
		   } else if (activity == "Guardando il concerto..." && currentTime == 44){
...
function longSleepAction(location){
	if (currentTime != sleepHour){
		bootbox.alert("<h3 class='text-center'> <i class='fa fa-exclamation-triangle' style='color: red;'></i>&nbsp; Non è ancora il momento di dormire!</h3>");
                nErr++;
                setTimeout(function(){
	    	dialog.modal('hide');
		    // Update date and time
...
function notifySatisfiedGoal(id){
	if (id != "sleepHour"){
        ...
        var sleepString = $("#sleepHour").text(); 
        id=sleepString;
    	showSnackbar("Hai soddisfatto un obiettivo!")
    	/*$("#"+id+" i").removeClass("fa-square-o");
    	$("#"+id+" i").addClass("fa-check-square-o");
...
function endExercise(){
  ...
  /*  
     if ((nCorrect/nTot)>=0.8){
            $("#summarySentence").append("<p id='loadperformance'>Caricamento risultati performance...</p> ");
	    solved=true;
            var d=nCorrect/nTot;
            console.log("sono dentro if :"+d+" corretti "+nCorrect+ "nTot "+nTot);
        //$("#summarySentence").text("Complimenti, sei riuscito a risolvere tutti gli obiettivi!");
	} else {
    ...
    //$("#loadperformace").load('/getperformance?exerciseid='+exerciseid+'&patientid='+patientid+'&ptime='+0,0+'&pcorrect='+nCorrect+"&pwrong="+nWrong+"&pmissed="+0+"&pmaxtime="+1,0+"&sessid="+sessid+"&difficulty="+difficulty+'&level='+level);
    var p;     
   ...
   //  $('#summarySentence').append('<a type="button" class="btn btn-default" href="pianificazione3phase3?difficulty='+difficulty+'&level='+level+'&patientid='+patientid+'&exerciseid='+exerciseid+'&idproblem='+idproblem+'&passed='+p+'&nWrong='+nWrong+'&nCorrect='+nCorrect+'&sessid='+sessid+'" >Ok</a>');
     if(p){
          $("#summarySentence").append("<p>Bene hai superato l'esercizio</p>");  
            console.log("sono dentro if Abagliati :"+nWrong+" corretti "+nCorrect+ "nTot "+nTot);
        //$("#summarySentence").text("Complimenti, sei riuscito a risolvere tutti gli obiettivi!");
	} else {
            console.log("sono dentro else");
            solved=false;
      ...
      console.log("elementi ok "+nCorrect+"obiettivi sbagliati "+nWrong+" perf ");
    //$("#unsolvedGoalsList").append("<li id='fine'></li>")
    
    $('#fine').append('<a type="button" class="btn btn-default" href="pianificazione3phase3?difficulty='+difficulty+'&level='+level+'&patientid='+patientid+'&exerciseid='+exerciseid+'&idproblem='+idproblem+'&passed='+p+'&nWrong='+nWrong+'&nCorrect='+nCorrect+'&sessid='+sessid+'" >Ok</a>');
    //$('#endExerciseModal').on('hidden.bs.modal', function () {
   //    'pianificazione3phase3?difficulty='+difficulty+'&level='+level+'&patientid='+patientid+'&exerciseid='+exerciseid+'&idproblem='+idproblem+'&passed='+p+'&nWrong='+nWrong+'&nCorrect'+nCorrect+'&sessid='+sessid;
   //});
     
     });
     
    
    // console.log("eccomi00"+prova);
    
  
 /*  
    $('#endExerciseModal').on('hidden.bs.modal', function () {
       'pianificazione3phase3?difficulty='+difficulty+'&level='+level+'&patientid='+patientid+'&exerciseid='+exerciseid+'&idproblem='+idproblem+'&passed='+solved+'&nWrong='+nWrong+'&nCorrect'+nCorrect+'&sessid='+sessid;
       console.log("eccomi "+nCorrect+"obiettivi sbagliati "+nWrong+" perf ");
