  ...
  <line x1="7" y1="12" x2="17" y2="12"  fill="none" stroke-width="3" stroke-linecap="round" />
	...
	var pasteActions = {
		range:function(data){
			...
			if(range){
				...
				startCell = bounds.startGame;
				
				if(bounds.startGame === bounds.end){
					singleCell = true;
				}
	...
	class SelectRange extends Module {
		...
		navigate(jump, expand, dir) {
			...
			if (this.ranges.length > 1) {
				this.ranges = this.ranges.filter((range) => {
					if (range === this.activeRange) {
						range.setEnd(range.startGame.row, range.startGame.col);
						return true;
					}
					range.destroy();
					return false;
			...
			rangeEdge = expand ? range.end : range.startGame;
			...
			if(jump){
				switch(dir){
					case "left":
						nextCol = this.findJumpCellLeft(range.startGame.row, rangeEdge.col);
						break;
					case "right":
						nextCol = this.findJumpCellRight(range.startGame.row, rangeEdge.col);
						break;
					case "up":
						nextRow = this.findJumpCellUp(rangeEdge.row, range.startGame.col);
						break;
					case "down":
						nextRow = this.findJumpCellDown(rangeEdge.row, range.startGame.col);
						break;
				}
			}else {
				if(expand){
		...
		getActiveCell() {
			return this.getCell(this.activeRange.startGame.row, this.activeRange.startGame.col);
		}
