...
<html>
    <head>
        ...
        <script>
            ...
            function getDirectionIcon(direction){
                if(direction>0)
                    return '<span class="glyphicon glyphicon-arrow-up padding-arrow" aria-hidden="true"></span>';
                if(direction<0)
                    return '<span class="glyphicon glyphicon-arrow-down padding-arrow" aria-hidden="true"></span>';
                if(direction==0)
                    return '<span class="glyphicon glyphicon-resize-horizontal padding-arrow" aria-hidden="true"></span>';
                
                return "";
            }
            function addChangedColor(changed, level, oldlevel){
                if(changed && oldlevel!==level){
                    return "red";
                }else{
                    return  "";
                }
            ...
            function addChangedDifficulty(changed,level,oldlevel){
                if(changed && oldlevel!==level){
                    return "cambiato da "+oldlevel;
                }
                else {
            ...
            function autoRefresh(){
               setTimeout(function(){
                    $.get("patientlivejson",{"id":${patient.id}},function(data){
            ...
            function insertHistory(h){
                if(h.length<=0){
                    //$("#infomessage").html("Nessun dato per il paziente scelto!");
            $("#infomessage").html("Nessun dato per il partecipante scelto!");
            ...
            $("#infomessage").show();
                ...
                /*$("#history").html('<thead><tr>'+
                        '<th>Nome</th>'+
                        '<th>Livello di difficolt&agrave;</th>'+
                        '<th>Performance</th>'+
                        '<th>Ultimo accesso</th>'+
                    '</tr></thead><tbody id="thistory"></tbody>');*/
                $("#thistory").html("");
                    ...
                    $("#thistory").append(
                            '<tr >'+
                                ...
                                '<td class="col-sm-6">'+
                                    ...
                                    '<input id="" class="short '+addChangedColor(h[i].changed, h[i].level, h[i].oldlevel)+'" type="text" readonly value="'+h[i].level+'/'+h[i].maxdiff+'" historyid="'+h[i].historyid+'"/>'+
                                    ...
                                    '<br />'+
                                    '<p class="infodiff">('+addChangedDifficulty(h[i].changed, h[i].level, h[i].oldlevel)+')</p>'+
                                '</td>'+
                                '<td class="col-sm-6">'+
    ...
    <body>
        <div class="container">
            ...
            <div class="well">
                <div><h2>Esercizi assegnati a ${patient.name} ${patient.surname}</h2></div>
                <div class="row">
        ...
        <jsp:include page="modal-pat-live.jsp"/>                
