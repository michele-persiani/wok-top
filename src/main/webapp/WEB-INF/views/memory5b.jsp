<%-- 
    Document   : memory5b
    Created on : Feb 8, 2016, 17:42:00 AM
    Author     : sara
--%>
    ...
    <body>
        <div class="container">
            ...
            <div id="exElements" class="well well-sm">
             ...
             <c:set var="nRigheCol" value="${nRigheCol}"/> 
             <c:set var="maxEle" value="${nRigheCol*nRigheCol}"/>
            <c:set var="caso" value="0"></c:set>
            <c:set var="nElemVuoti"   value="0"/>
            <c:set var="maxElemen"  value="${(nRigheCol*nRigheCol)-nelements}"/>
            
            <c:set var="nelementi"   value="${nelements}"/>
            <c:set var="sino" value="0"/>
            <form id="form" role="form">
                   <table class="table table-bordered">    
                        <!--c:forEach var="element" items="${exElementList}"-->
                         <c:forEach var="i" begin="0" end="${maxEle-1}">
                             
                            <c:if test="${i%nRigheCol==0}">
                            <tr>
                            </c:if>
                                 <c:set var="caso" value="<%= (int) (java.lang.Math.random() * 2) %>"/>
                           
                                 <c:if test="${caso==1||nElemVuoti>=maxElemen}">  
                            <td>
                                <div class="checkbox">
                                    <input type="checkbox" disabled=true" id="i${i}" value="${exElementList[i-nElemVuoti].id}" onclick="checkAnswer()"/>
                                    <label for="i${i}">
                                        <h3 id="e-i${i}" style="color:red">${exElementList[i-nElemVuoti].eldescr}</h3>
                                    </label>  
                                          </div> 
                            </td>
                                <c:set var="nelementi"   value="${nelementi-1}"/>
                               <c:set var="sino" value="1"/>
                            </c:if>  
                            <c:if test="${(caso==0 && nElemVuoti<maxElemen) || (nelementi==0&&sino==0)}">
                            <td> 
                                <div class="checkbox"> 
                                 <input type="checkbox" disabled=true" id="i${i}"  onclick="checkAnswer()" />
                                    <label for="i${i}">
                                        <h3 id="e-i${i}" style="color:red">?</h3>
                                    </label>     
                                </div>  
                            </td>
                         <c:set var="nElemVuoti" value="${nElemVuoti+1}"/>
                        
                              </c:if>
                             <c:if test="${i%nRigheCol==nRigheCol-1}">
                            </tr>
                            </c:if>
                                        <c:set var="sino" value="0"/>                        
                        </c:forEach>
                ...
                <div class="well well-sm">
                    <div class="row">
                        ...
                        <div class="col-sm-2">
                            <h3 class="pull-right"><h3> Dov'&egrave;?</h3>
                        </div>
                        <div class="col-sm-8">
                            <h3 class="pull-right"><h3>Clicchi sulla posizione esatta in</h3>
                        </div>
                        <div class="col-sm-1">
        ...
        <script src="resources/assets/js/ie10-viewport-bug-workaround.js"></script>
         ...
         <script> 
            ...
            var maxElenti=${nRigheCol} *${nRigheCol};
        ...
        //    var elements = document.getElementById("form").elements;
            ...
            function myTimer() {
                ...
                if (startTimer===0) {
                    ...
                    for (var i = 0; i<maxElenti; i++) {
                        console.log(" dentro il for "+document.getElementById("#e-i"+i));
                        
                         document.getElementById("e-i"+i).innerHTML="<h1>?</h1>";
                         console.log(" dentro il for dopo "+document.getElementById("#e-i"+i));
            ...
            function checkAnswer() {
                ...
                console.log("dentro checkAnswer "+dTime);
                 console.log("controllo varibili  checked"+$("#i"+i).type+" value "+$("#i"+i).value+" target "+${target}[j]);
                for (var i = 0; i<maxElenti; i++) {
                    if (document.getElementById('i'+(i)).type === "checkbox") {
                        if (document.getElementById('i'+(i)).checked) {
                           
                        console.log(" cccc cheched "+dTime);
                             
                            if(document.getElementById('i'+(i)).value == ${target}[j]) {
                                nCorrect = nCorrect+1;
            ...
            function calculateResult() {
                $.get("getperformance",
                {
                    ...
                    "pwrong": nWrong,
                    "pmissed": nMissed,
                    "maxtime": Math.round(${time}*${nelements}*nTarget),
                    "sessid": ${sessid},
